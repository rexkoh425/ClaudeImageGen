...
26. Use `--save-candidates 4` for complex, subjective, or low-confidence prompts. Inspect `candidates.json` and the `candidates/candidate-*.png` files when several candidates have close scores, when the best score looks visually worse than an alternative, or when deciding which image to refine next.
