...
- The default scorer and captioner are lightweight local prompt/reference proxies with explicit cosine and caption backchecking. Optional `transformers-clip`, `transformers-siglip`, `transformers-dinov2`, and `transformers-blip` are only for stronger model-backed checks when dependencies and weights are already available or the user accepts downloading them.
