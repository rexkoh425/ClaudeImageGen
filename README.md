...
- `metadata.json`: prompt, score, dimensions, seed, detected objects/colors/style/mood words, extracted `reference_palette` / `initial_palette`, threshold result, `score_details.cosine_score`, `initial_similarity_details` for image-to-image continuity, `image_caption`, `caption_similarity_score`, caption missing/unexpected evidence, candidate artifact paths, recommended candidate fields including `recommended_candidate_aesthetic_score`, local refinement actions, `refinement_delta` for refine runs, and `revision_hints` when Claude should revise a weak scene plan.
- `quality-report.json`: first-stop readiness report for Claude Code, combining prompt alignment, caption alignment, continuity, reference alignment, candidate recommendation, refine-run `refinement_delta` and `refinement_guidance`, parent-child `visual_comparison`, failed checklist items from Claude vision, and concrete `next_actions`.
- `verification-report.json`: `verify` summary with `device_summary` for CPU/GPU evidence, `image_summary` for nonblank image checks, per-case image variance statistics, requested sizes, strong-model backends, and artifact paths.
- `critique-request.json`: Claude-vision judge request for the next iteration, including image/metadata/report paths, the prompt, quality status, a VQAScore-style `visual_checklist` for requested objects/colors plus style/mood checks, allowed edit actions, and the JSON response schema with `element_checks` to fill before running `refine --critique`.
- `comparison-request.json`: refine-only Claude-vision request that asks Claude Code to open the parent and child image side by side, judge whether the child improved while preserving identity/layout, and return follow-up edits to pass into `refine --comparison` when it regressed.
- `progress.csv`: score per iteration.
- `candidates.json`: optional ranked candidate index when `--save-candidates N` is passed, including score details, captions, caption missing/unexpected evidence, deterministic `aesthetic_score` / `aesthetic_details`, and `selection_score` / `selection_reasons` for each candidate.
...
The refined `metadata.json` includes `refined_from`, `parent_image`, `parent_metadata`, `refinement_lineage_depth`, `initial_similarity_score`, `initial_similarity_details`, `parent_total_score`, `parent_quality_score`, `parent_caption`, `parent_caption_similarity_score`, `refinement_delta`, `refinement_guidance`, `comparison_request`, `visual_comparison` when a filled comparison is supplied, and `parent_candidate_*` fields when `--candidate-rank` is used. Use `--candidate-rank auto` to start from the saved candidate with the strongest combined total score, caption similarity, reference score, deterministic `aesthetic_score`, and caption-diagnostic penalties. Start each iteration by reading `quality-report.json`, `critique-request.json`, and, for refine runs, `comparison-request.json`; fill the critique request's `expected_response` into `critique.json` after viewing `image.png`, including one `element_checks` answer for each `visual_checklist` item, then pass it to `refine --critique critique.json`. For parent/child regressions, fill the comparison request's `expected_response` into `comparison.json` after viewing both images, then pass it to `refine --comparison comparison.json` so `follow_up_edits` are applied and recorded under `visual_comparison`. Use `refinement_guidance.priority_axes` first to decide which axis to fix next, then inspect `refinement_delta.total_score_delta`, `refinement_delta.quality_score_delta`, and `refinement_delta.caption_similarity_delta` to compare the refined output against its parent. Use `initial_similarity_details.continuity_score`, `image_cosine_score`, `luminance_ssim_score`, `multiscale_luminance_ssim_score`, `region_similarity_scores`, `weakest_continuity_region`, `edge_cosine_score`, and `color_histogram_score` to confirm continuity with the previous image while `score_details.cosine_score`, `caption_similarity_score`, and `reference_score` track prompt/reference alignment.

Use `--save-candidates N` on either `generate` or `refine` when Claude Code should inspect alternatives instead of trusting only the best-scored final image. The generator writes `candidates.json`, `candidates/candidate-*.png`, and `candidates/contact-sheet.png`; each index entry records rank, iteration, image path, total/text/reference scores, score details, threshold status, candidate caption, caption similarity, candidate-level missing/unexpected prompt evidence, deterministic `aesthetic_score` / `aesthetic_details`, and `selection_score` / `selection_reasons`. The run metadata also records `recommended_candidate_rank`, `recommended_candidate_image`, `recommended_candidate_score`, `recommended_candidate_aesthetic_score`, and `recommended_candidate_reasons`. If a lower-ranked candidate is visually stronger, run `refine --from-dir <parent> --candidate-rank <rank>`; otherwise use `refine --from-dir <parent> --candidate-rank auto` to let the saved selection score choose the next initial image.
...
When refining from an initial image with `--similarity-backend transformers-clip`, `initial_similarity_details` also includes `clip_image_cosine_score`; with `--similarity-backend transformers-siglip`, it includes `siglip_image_cosine_score`; with `--continuity-backend transformers-dinov2`, it includes `dinov2_image_cosine_score`. These are image-embedding cosines between the new image and the selected parent image. The local continuity score always includes `multiscale_luminance_ssim_score` so parent-child structure is checked at multiple resolutions instead of only one pixel scale. It also records a 3x3 `region_similarity_scores` grid plus `weakest_continuity_region` and `weakest_continuity_region_score`, giving Claude Code a concrete area to inspect when a child image preserves the subject globally but drifts in one region. The final `initial_similarity_score` blends local continuity with the optional model-backed image cosine, giving Claude Code a stronger continuity signal when cached local model weights are available.
...
When `quality-report.json` reports `status: "revise"`, its `next_actions` list is the primary checklist for the next scene-plan edit. `critique-request.json` is the paired Claude-vision checklist: open `image.png`, compare it against the prompt and report, answer each VQAScore-style `visual_checklist` object/color question plus style/mood checks, fill the response schema as `critique.json` with `element_checks`, then run `refine --critique critique.json` so structured visual edits are recorded and applied. Failed checklist items are copied into the `visual_judgement` check and promoted into `next_actions`, so absent or low-confidence requested elements become explicit `Judge:` revision work. `refine --critique` also creates conservative checklist-derived edits: missing checked objects can add visible default objects or cloud banks, weak checked colors can raise saturation/contrast and append the matching palette color, and weak style/mood checks can tune contrast, bloom, vignette, warmth, or saturation. On refine runs, `comparison-request.json` adds the parent-child check: open both images side by side, judge whether the child improved while preserving identity, layout, palette, and subject continuity, save the filled response as `comparison.json`, then run `refine --comparison comparison.json` when the next run should apply its follow-up edits. Filled comparisons are recorded as `visual_comparison`, added to `quality-report.json` as a `visual_comparison` check, and converted into `Comparison:` next actions when regressions are listed. `metadata.json` still includes the underlying `revision_hints` with concrete scene-plan changes for Claude Code to make next, such as adding missing objects, strengthening requested colors, increasing contrast, or moving closer to a reference image. Refine reports include `refinement_delta` plus `refinement_guidance` in both `metadata.json` and `quality-report.json`; `refinement_guidance.priority_axes` labels prompt-alignment, quality, caption-alignment, and continuity regressions with severity and an action, so Claude Code can pick the next scene-plan edit without hand-interpreting every score. If a scene plan is provided, the generator also performs bounded local auto-refinement between iterations: it can add missing prompt objects such as clouds, add a matching cloud layer, and increase contrast or saturation when the local scorer shows weak evidence. If a reference or initial image is provided, `reference_palette` and `initial_palette` expose extracted hex colors that Claude can fold into the next scene plan. `image_caption`, `caption_similarity_score`, `lexical_caption_similarity_score`, `semantic_caption_similarity_score`, `caption_missing_objects` / `caption_missing_colors`, `initial_similarity_details.multiscale_luminance_ssim_score`, and `weakest_continuity_region` add direct visual and structural checks before revising the next scene plan. The intent is to keep the expensive semantic iteration in Claude Code while local work remains deterministic rendering, scoring, caption backchecking, quality reporting, critique request generation, parent-child comparison requests, and targeted refinement.
...
`claude-imagegen verify` writes `verification-report.json`, generates each requested size, saves candidate artifacts, runs one complex planned scene case, runs one `refine --candidate-rank auto` case, and records every output's `metadata.json`, `quality-report.json`, `critique-request.json`, plus refine-case `comparison-request.json` and `refinement_delta`. Each case now records `image_stats`, `image_nonblank`, and `image_variance_sum`; a blank or uniform image fails the case, and the top-level `image_summary` reports nonblank and blank case counts. The top-level `device_summary` aggregates effective CPU/GPU devices by similarity, continuity, caption, and caption-similarity role, plus the backends used, so strong runs can prove whether CUDA was actually used. The complex planned scene case writes a rich `scene-plan.json` with gradients, motifs, textures, materials, terrain, reflections, warps, atmosphere, veils, lights, beams, clouds, shadows, and focus; the report includes scene-plan feature counts and fails the case if that complexity is not present. Add `--strong-model --strong-model-device auto` to include model-backed similarity plus BLIP verification cases when local `torch`, `transformers`, and model weights are available. Use `--strong-size WIDTHxHEIGHT` once or repeatedly to run those strong cases at larger or different dimensions than the default first `--size`. Use `--strong-similarity-backend transformers-siglip --similarity-model google/siglip-base-patch16-224` to run strong cases with SigLIP instead of CLIP. Add `--strong-continuity-backend transformers-dinov2 --continuity-model facebook/dinov2-base` to include matching strong refine cases with DINOv2 parent-image continuity. Add `--caption-similarity-backend transformers-sentence --caption-similarity-model sentence-transformers/all-MiniLM-L6-v2` to score BLIP captions against prompts with sentence embeddings.
