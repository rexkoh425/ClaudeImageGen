...
- `quality-report.json`: first-stop readiness report for Claude Code, combining prompt alignment, caption alignment, continuity, reference alignment, candidate recommendation, refine-run `refinement_delta`, failed checklist items from Claude vision, and concrete `next_actions`.
...
- `comparison-request.json`: refine-only Claude-vision request that asks Claude Code to open the parent and child image side by side, judge whether the child improved while preserving identity/layout, and return follow-up edits when it regressed.
